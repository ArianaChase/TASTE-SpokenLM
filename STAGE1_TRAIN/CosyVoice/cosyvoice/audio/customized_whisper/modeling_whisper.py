#### THIS IS A COPY VERSION FROM HUGGINGFACE's `modeling_whisper`
# coding=utf-8
...
class WhisperAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value and is_updated:
            ...
            # customized for key value separate cross-attn
            if isinstance(current_states, dict):
                states_for_key = current_states['states_for_key']
                states_for_val = current_states['states_for_val']
                # adopt separation
                key_states = self._shape(self.k_proj(states_for_key), -1, bsz)
                value_states = self._shape(self.v_proj(states_for_val), -1, bsz)
            else:
...
class WhisperDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        encoder_hidden_states: Optional[Union[torch.Tensor, Dict[str, torch.Tensor]]] = None, # allow key value separation by passing key and value by dict
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states, cross_attn_weights, cross_attn_present_key_value = self.encoder_attn(
                hidden_states=hidden_states,
                key_value_states=encoder_hidden_states, # allow key value separation
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                ...
                [What are attention masks?](../glossary#attention-mask)
            encoder_hidden_states (`torch.FloatTensor` of shape `(batch_size, encoder_sequence_length, hidden_size)`, *optional*, or `Dict[str, torch.FloatTensor]` for separating key and value source hidden states):
